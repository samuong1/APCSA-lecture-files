...
int main()
{
    ...
    double length; double m_rod; double d_rod; double eth_fusion = 334; double tempi = 0; double temp;
    double melt_rate; double time;

    cin >> length >> m_rod >> d_rod >> tempi >> melt_rate >> time;
    double vol = m_rod / d_rod;
    double radius = sqrt(vol / ((3.14) * (length)));
    double area = 3.14 * radius * radius;
    double heatflowed = eth_fusion * melt_rate;
    double k = (heatflowed * length) / (area * (tempi - temp) * time * 60);
