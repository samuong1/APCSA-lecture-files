...
#define afastio ios::sync_with_stdio(0); cin.tie(0); freopen("Treasure.in", "r", stdin); 
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        char dir; ll length;
        cin >> dir >> length;
        directions[dir].push_back(length);
    }
    char dir[4] = {'N', 'S', 'E', 'W'};
    for (int i = 0; i < 4; i++)
    {
        vll &s = directions[dir[i]];
        sort(s.begin(), s.end());
    ...
    for (int i = 0; i < M; i++)
    {
        ...
        {
            vll s = directions[dir[i]];
            ll left = 0; ll right = s.size() - 1;
            while (left <= right)
            {
                ll mid = left + (right - left) / 2;
                ll val = s[mid] - (mid > 0 ? s[mid - 1] : 0);
                if (val <= T)
                {
                    left = mid + 1;
                } 
            ...
            if (right >= 0)
            {
                x += dirx[i] * s[right]; y += diry[i] * s[right];
            }
        }
        cout << x << " " << y << "\n";
    }
